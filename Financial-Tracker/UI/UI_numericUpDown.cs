...
namespace ExpenseTracker.UI
{
    ...
    using System.Runtime.InteropServices;
    using System.Windows.Forms;
    public class UI_numericUpDown : NumericUpDown
    {
        ...
        private class UpDownButtonRenderer : NativeWindow
        {
            ...
            {
                if (m.Msg == 0xF /*WM_PAINT*/ && ((UI_numericUpDown)updown.Parent).BorderStyle == BorderStyle.FixedSingle)
                {
                    ...
                    using (var g = Graphics.FromHdc(s.hdc))
                    {
                        ...
                        using (var backBrush = new SolidBrush(enabled ? ((UI_numericUpDown)updown.Parent).BackColor : SystemColors.Control))
                        {
                        ...
                        {
                            using (var b = new SolidBrush(((UI_numericUpDown)updown.Parent).ButtonHighlightColor))
                            {
                                if (r1.Contains(p))
                            ...
                            using (var pen = new Pen(((UI_numericUpDown)updown.Parent).BorderColor))
                            {
                                g.DrawLines(pen,
                        ...
                        g.FillPolygon(Brushes.White, GetUpArrow(r1));
                        g.FillPolygon(Brushes.White, GetDownArrow(r2));
                    }
                    m.Result = (IntPtr)1;
                    base.WndProc(ref m);
